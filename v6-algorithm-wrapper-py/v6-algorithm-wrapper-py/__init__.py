...
def new_data_decorator(func: callable, *args, **kwargs) -> callable:
    ...
    def decorator(*args, mock_data: list[pd.DataFrame] = None, **kwargs) -> callable:
        ...
        for cohort_name in cohort_names:
            ...
            data_frames.append(pd.read_parquet(f"/mnt/data/{cohort_name}.parquet"))
...
def summary(
    ...
    # for cohort_name, result in variance_results.items():

    #     all_cohort_results[cohort_name] = _add_sd_to_results(
    #         all_cohort_results[cohort_name], result, numerical_columns
    #     )
